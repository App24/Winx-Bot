...
export = () => {
    BotUser.on("messageCreate", async (message) => {
        ...
        try {
            ...
            if (xpData.length >= serverInfo.document.maxMessagePerMinute) return;
            ...
            const xp = Math.ceil((Math.min(message.content.length, serverInfo.document.maxMessageLength) / serverInfo.document.maxMessageLength) * serverInfo.document.maxXpPerMessage);
